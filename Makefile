...
GO_BUILD := $(Q)go build
PKGS := $(shell go list ./...)
DEPPY_VERSION :=  $(shell git describe --match 'v[0-9]*' --tags --always)

# Binary build options
export KUBERNETES_VERSION=v0.22.2

# Container build options
export IMAGE_REPO ?= quay.io/operator-framework/deppy-operator
...
# Build Targets #
#################
.PHONY: tidy generate format lint verify build-cli build-container
...
VERSION_FLAGS=-ldflags "-X $(VERSION_PATH).GitCommit=$(GIT_COMMIT) -X $(VERSION_PATH).DeppyVersion=$(DEPPY_VERSION) -X $(VERSION_PATH).KubernetesVersion=$(KUBERNETES_VERSION)"
build-cli: ## Build the CLI binary. Specify VERSION_PATH, GIT_COMMIT, or KUBERNETES_VERSION to change the binary version. You may also specify BUILD_OS and BUILD_ARCH to change the build's binary.
	$(Q) CGO_ENABLED=0 GOOS=$(BUILD_OS) GOARCH=$(BUILD_ARCH) go build $(VERSION_FLAGS) -o ./deppy

build-container: BUILD_OS=linux
build-container: BUILD_ARCH=amd64
build-container: build-cli ## Build the Deppy container from the Dockerfile. Accepts IMAGE_REPO and IMAGE_TAG overrides.
...
deploy: generate ## Deploy the Deppy operator to the current cluster
	kubectl apply -f manifests

teardown: ## Teardown the Deppy operator to the current cluster
	kubectl delete -f manifests

run: build-container load-image deploy ## Run Deppy on local cluster

run-local: build-container load-image deploy ## Run Deppy on local environment with Dockerfile

run-e2e: run test-e2e ## Run Deppy and trigger the e2e tests for it

run-e2e-local: run-local test-e2e ## Run Deppy on local environment and trigger e2e tests for it using Dockerfile
